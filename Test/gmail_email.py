...
def send_email():
        ...
        Dosi_Number = config.get('General', 'dosi_number')
        todays_date = config.get('General','todays_date')
        #needs logic for existing return date
        Return_Date = config.get('General', 'return_date')
        if Return_Date == 'None':
                Return_Date_calculated = todays_date
        else:
                Return_Date_calculated = Return_Date

        os.chdir(local_path)
        data_unreturned = dataframe.return_dataframe_view1()
        #needs logic if no additional unreturned dosimeters!!!
